...
## 更新摘要
**所做更改**
- 更新了APIC初始化和配置部分，反映改进的中断控制器管理
- 增强了内核初始化顺序说明，包括新的启动序列
- 改进了设备初始化流程的描述
- 更新了中断处理机制的技术细节
- 添加了新的SMP支持和多处理器初始化信息
...
- 内核层：
  ...
  - 内核初始化流程：kernel/kernel.c、arch/x86/kernel/setup.c
...
- [arch/x86/kernel/idt.c:256-331](file://arch/x86/kernel/idt.c#L256-331)
...
**更新** 改进了APIC初始化流程，增加了更详细的错误处理和调试输出
...
**图表来源**
- [arch/x86/kernel/apic.c:37-180](file://arch/x86/kernel/apic.c#L37-L180)
- [arch/x86/kernel/apic.c:230-289](file://arch/x86/kernel/apic.c#L230-289)
- [arch/x86/kernel/apic.c:449-475](file://arch/x86/kernel/apic.c#L449-475)
...
**更新** 增强了软中断处理机制，改进了ksoftirqd线程管理和任务调度
...
- 默认处理：idt.c 中的 do_trap 打印寄存器、EIP 附近指令字节，内核态异常直接停机防止无限循环。
...
- [arch/x86/kernel/idt.c:173-231](file://arch/x86/kernel/idt.c#L173-231)
...
### 内核初始化顺序与设备初始化流程
**新增** 本节详细说明改进后的内核初始化顺序和设备初始化流程

- 内核启动序列：
  - `_kernel_init()` → `_init_gdt()` → `setup_arch()`
  - `setup_arch()` → `copy_multiboot_info()` → `setup_memery()` → `paging_init()`
  - `fbcon_init()` → `acpi_tables_init()` → `tty_set_buffer_base()`
- 设备初始化流程：
  - `platform_bus_init()` → `acpi_register_platform_devices()`
  - `i8042_init()` → `keyboard_init()` → `mouse_init()`
  - `pci_init()` → `block_dev_init()` → `buffer_cache_init()`
  - `ata_init()` → `sata_init()` → `usb_init()` → `uhci_init()`
- 图形系统初始化：
  - `drm_core_init()` → `drm_bochs_init()` → `drm_fb_helper_init()`
  - `drm_fb_test_image()` → `softirq_init()` → `smp_init()`

**更新** 改进了设备初始化顺序，确保依赖关系正确，提高了系统稳定性

**章节来源**
- [kernel/kernel.c:30-91](file://kernel/kernel.c#L30-L91)
...
APIC_C --> INT_C
KERNEL_C["kernel.c"] --> SETUP_C["setup.c"]
SETUP_C --> APIC_C
```
