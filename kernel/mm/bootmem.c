...
void __init reserve_bootmem(unsigned long addr, unsigned long size)
{
    ...
    if (starti < 0 || endi < 0 || start_pfn >= bdata->node_low_pfn || end_pfn > bdata->node_low_pfn || starti >= endi)
    {
        return;
    }
    for (int i = starti; i < endi; i++)
...
void *__init __alloc_bootmem(unsigned long size, unsigned long align, unsigned long goal)
{
    ...
    if (goal && goal >= bdata->node_boot_start && PFN_DOWN(goal) < bdata->node_low_pfn)
    {
...
unsigned long __init free_all_bootmem()
{
    ...
    ///2.释放bootmem位圖頁，添加到空闲链表
    page =   virt_to_page(bdata->node_bootmem_map);
    for (unsigned long i = 0; i < ((bdata->node_low_pfn-(bdata->node_boot_start >> PAGE_SHIFT))/8 + PAGE_SIZE-1)/PAGE_SIZE; i++,page++) {
		usablePageCount++;
		ClearPageReserved(page);
		set_page_count(page, 1);
		__free_page(page);
